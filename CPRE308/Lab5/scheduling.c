...
int first_come_first_served(process proc[], int t)
{
  int minProcArrivalTime = INFINITY; // Set to a really high number initially
  int minProc = -1;

  if (NUM_PROCESSES < 0) // Edge case for if there are no processes
    return -1;

  for (int i = 0; i < NUM_PROCESSES; i++)
  {
    if (proc[i].arrivaltime < minProcArrivalTime && proc[i].finished == 0 && proc[i].arrivaltime <= t)
    {
      minProcArrivalTime = proc[i].arrivaltime;
      minProc = i;
    }
  }
...
int shortest_remaining_time(process proc[], int t)
{
  int shortestRemainingTime = INFINITY; // Set to a really high number initially
  int shortestRemainingProc = -1;
  ...
  for (int i = 0; i < NUM_PROCESSES; i++)
  {
    if (proc[i].remainingtime < shortestRemainingTime && proc[i].finished == 0 && proc[i].arrivaltime <= t)
    {
      shortestRemainingTime = proc[i].remainingtime;
      shortestRemainingProc = i;
    }
  }

  return shortestRemainingProc;
}

int round_robin(process proc[], int t)
{
  // HINT: consider using a static variable to keep track of the previously scheduled process
  // One second of Quantum Time
  ...
  {
    int currProc = (lastProc + i) % NUM_PROCESSES;
    if (proc[currProc].arrivaltime <= t && proc[currProc].finished == 0)
    {
      lastProc = currProc;
      return currProc;
    }
...
{
  static int lastProc = -1;
  int highestPriority = -1; // Set to -1 and change later when next highest comes in

  if (NUM_PROCESSES < 0) // Edge case for if there are no processes
    return -1;

  // Find highest priority first
  for (int i = 0; i < NUM_PROCESSES; i++)
  {
  ...
  // Same as round robin but with extra check for priority
  for (int i = 1; i <= NUM_PROCESSES; i++)
  {
    int currProc = (lastProc + i) % NUM_PROCESSES;
    if (proc[currProc].arrivaltime <= t && proc[currProc].finished == 0 && proc[currProc].priority == highestPriority)
    {
      lastProc = currProc;
      return currProc;
    }
