...
void decodeBootSector(struct BootSector * pBootS, unsigned char buffer[])
{
    ...
    for(i = 0; i < 8; i++)
	{
		pBootS->sName[i] = buffer[i + 3];  // The Name starts at byte 3
	}
	pBootS->sName[8] = '\0';  // We need to null terminate the string

	// Read bytes/sector and convert to big endian
    pBootS->iBytesSector = endianSwap(buffer[11], buffer[12]);  // Bytes 11-12

	// Read sectors/cluster, Reserved sectors and Number of Fats
    pBootS->iSectorsCluster = buffer[13];  // Byte 13
	pBootS->iReservedSectors = endianSwap(buffer[14], buffer[15]);  // Bytes 14-15
	pBootS->iNumberFATs = buffer[16];  // Byte 16

	// Read root entries, logicical sectors and medium descriptor
    pBootS->iRootEntries = endianSwap(buffer[17], buffer[18]);  // Bytes 17-18
	pBootS->iLogicalSectors = endianSwap(buffer[19], buffer[20]);  // Bytes 19-20
	pBootS->xMediumDescriptor = buffer[21];  // Byte 21

	// Read and covert sectors/fat, sectors/track, and number of heads
    pBootS->iSectorsFAT = endianSwap(buffer[22], buffer[23]);  // Bytes 22-23
	pBootS->iSectorsTrack = endianSwap(buffer[24], buffer[25]);  // Bytes 24-25
	pBootS->iHeads = endianSwap(buffer[26], buffer[27]);  // Bytes 26-27

	// Read hidden sectors
	pBootS->iHiddenSectors = endianSwap(buffer[28], buffer[29]);  // Bytes 28-29
...
void parseDirectory(int iDirOff, int iEntries, unsigned char buffer[])
{
    ...
    for(i = 0; i < (iEntries); i = i + 1)   {
		...
		//Display Attributes
		parseAttributes(string, buffer[i*32 + 0x0B]); // 1 byte offset at 0x0B
		printf("%s\t", string);
		
		//Display Time
		parseTime(string, endianSwap(buffer[i*32 + 0x16], buffer[i*32 + 0x17])); // 2 bytes at offset 0x16, for little endian
		printf("%s\t", string);
		
		//Display Date
		parseDate(string, endianSwap(buffer[i*32 + 0x18], buffer[i*32 + 0x19])); // 2 bytes at offset 0x18, for little endian
		printf("%s\t", string);
		
		//Display Size
        // We need to construct a 4 byte size from the individual bytes
        unsigned int fileSize = buffer[i*32 + 0x1C] + // Least Significant Byte (LSB)
		(buffer[i*32 + 0x1D] << 8) + // 2nd Byte
		(buffer[i*32 + 0x1E] << 16) + // 3rd Byte
		(buffer[i*32 + 0x1F] << 24); // Most Significant Byte (MSB)
        // Then we print the fileSize
		printf("%d\n", fileSize);
    }
