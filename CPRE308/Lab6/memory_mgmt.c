...
int PRAlgo_FIFO(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	int frame_to_evict = 0; // index for the frame we want to evict, defaults to the first frame
	int i;
	int earliest_arrival = PageFrames[0].time_of_arrival; // track the earliest arrival time
	
	// loop through all the frames and find the earliest arrival time
	for(i = 1; i < num_frames; i++)
	{
		// if current frame arrived earlier than our current earliest
		if(PageFrames[i].time_of_arrival < earliest_arrival)
		{
			earliest_arrival = PageFrames[i].time_of_arrival; // update earliest
			frame_to_evict = i; // update the frame we need to evict
		}
	}

	return frame_to_evict; // return index for oldest frame (First In)
...
int PRAlgo_LRU(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	int frame_to_evict = 0; // index for the frame we want to evict, defaults to the first frame
	int i;
	int earliest_access = PageFrames[0].time_of_access; // track the least recently accessed time
	
	// loop through all frames to find least recently used
    for(i = 1; i < num_frames; i++)
    {
        // if current frame was accessed earlier than our current earliest
        if(PageFrames[i].time_of_access < earliest_access)
        {
            earliest_access = PageFrames[i].time_of_access;  // update earliest access time
            frame_to_evict = i;  // update the frame we need to evict
        }
    }
    
    return frame_to_evict;  // return index for least recently used frame
}
...
int PRAlgo_OPT(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	int frame_to_evict = 0;  // index for the frame we want to evict, defaults to the first frame
    int i, j;
    int farthest_use = -1;  // track the farthest in the future use time
    
    // loop through all frames to find which will be used farthest in the future
    for(i = 0; i < num_frames; i++)
    {
        int next_use = num_accesses;  // assuming that the page is never used again by setting to max value
        
        // loop through future accesses to find when this page is next used
        for(j = current_access + 1; j < num_accesses; j++)
        {
            // if this future access matches the current frame's page
            if(PageAccesses[j] == PageFrames[i].page_id)
            {
                next_use = j;  // found next use, record the time
                break;  // no need to wait anymore
            }
        }
        
        // if this page is used farther in future than current farthest, then we update
        if(next_use > farthest_use)
        {
            farthest_use = next_use;  // update farthest use time
            frame_to_evict = i;  // update the frame we need to evict
        }
    }

	return frame_to_evict;  // return index of page used farthest in future (or never)
